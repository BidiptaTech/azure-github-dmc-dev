        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="AttractionCloseDate" method="POST" action="{{ route('attraction_close_dates') }}">
                        @csrf
                        <input type="hidden" name="attraction_id" value="{{$attraction_id}}">
                        
                        <!-- Closed Days -->
                        <div class="mb-3">
                            <label for="attraction_closed_days"><strong>Closed Days</strong></label>
                            <select id="attraction_closed_days" class="form-control select2" name="attraction_closed_days[]" multiple required>
                                @php
                                    $closedDays = json_decode($attraction->close_days, true) ?? [];
                                @endphp
                        ...
                        <!-- Closed Dates -->
                        <div class="mb-3">
                            <label for="attraction_holiday_dates"><strong>Holiday Dates</strong></label>
                            <input type="text" class="form-control flatpickr-input" id="attraction_holiday_dates"
                                   name="attraction_holiday_dates" placeholder="Select holiday dates" required>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const closedDays = @json(json_decode($attraction->close_days, true) ?? []);
        const closedDates = @json(explode(',', $attraction->close_dates ?? ''));

        // Initialize Flatpickr for selecting close dates
        flatpickr("#attraction_holiday_dates", {
            mode: "multiple",
            dateFormat: "Y-m-d",
        ...
        $('#attraction_closed_days').select2({
