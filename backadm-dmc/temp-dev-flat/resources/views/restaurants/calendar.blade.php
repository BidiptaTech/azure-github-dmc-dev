        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="RestaurantCloseDate" method="POST" action="{{ route('restaurant_close_dates') }}">
                        @csrf
                        <input type="hidden" name="restaurant_id" value="{{$restaurant_id}}">
                        
                        <!-- Closed Days -->
                        <div class="mb-3">
                            <label for="restaurant_closed_days"><strong>Closed Days</strong></label>
                            <select id="restaurant_closed_days" class="form-control select2" name="restaurant_closed_days[]" multiple required>
                                @php
                                    $closedDays = is_string($restaurant->close_days) ? json_decode($restaurant->close_days, true) : ($restaurant->close_days ?? []);
                                @endphp
                        ...
                        <!-- Closed Dates -->
                        <div class="mb-3">
                            <label for="restaurant_holiday_dates"><strong>Holiday Dates</strong></label>
                            <input type="text" class="form-control flatpickr-input" id="restaurant_holiday_dates"
                                   name="restaurant_holiday_dates" placeholder="Select holiday dates" required>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const closedDays = @json(is_string($restaurant->close_days) ? json_decode($restaurant->close_days, true) : ($restaurant->close_days ?? []));
        const closedDates = @json(explode(',', $restaurant->close_dates ?? ''));

        // Initialize Flatpickr for selecting close dates
        flatpickr("#restaurant_holiday_dates", {
            mode: "multiple",
            dateFormat: "Y-m-d",
        ...
        $('#restaurant_closed_days').select2({
