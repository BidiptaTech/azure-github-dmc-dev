...
const PackageHeader = ({ packageData }) => {
  ...
  const allImages = packageData.main_image ? 
    [packageData.main_image, ...(packageData.gallery_images || [])] : 
    [];
  ...
  return (
    ...
    >
      ...
      <Box sx={{
        position: 'relative',
        height: '300px',
        backgroundImage: `url(${allImages.length > 0 ? allImages[currentImageIndex] : ''})`,
      ...
      <Box sx={{ p: 2, bgcolor: '#f5f5f5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box>
          <Typography variant="body2">
            Available: {new Date(packageData.start_date).toLocaleDateString('en-GB', {
              weekday: 'short',
              day: 'numeric',
              month: 'short',
              year: '2-digit',
            }).replace(',', '')} - {new Date(packageData.expire_date).toLocaleDateString('en-GB', {
              weekday: 'short',
              day: 'numeric',
              month: 'short',
              year: '2-digit',
