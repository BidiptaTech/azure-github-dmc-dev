...
const PricingSummary = ({ totalPrice }) => {
  ...
  // Get restaurant bookings data to access transport info
  const restaurantBookings = useSelector((state) => state.restaurants?.restaurantBookings || []);
  const bookingData = restaurantBookings?.[0]?.data?.[0] || {};
  
  // Get transport information
  const transport = bookingData.transport;
  const mealPrice = bookingData.mealPrice || 0;
  const transportPrice = bookingData.transportPrice || 0;
  const adultCount = bookingData.adultCount || 0;
            ...
            <div className="row y-gap-5 justify-between pt-5">
              <div className="col-auto">
                <div className="text-15">
                  ...
                  {transport.transport_type === 'shared' && ` × ${totalPax} pax`}
                </div>
              </div>
              <div className="col-auto">
                <div className="text-15">
                  SGD {formatPrice(transportPrice)}
                </div>
              </div>
            </div>
          )} */}
          ...
          <div className="row y-gap-5 justify-between pt-5">
            ...
            <div className="col-auto">
              <div className="text-15 fw-500">
                ...
                {currencyCode !== 'SGD' && currencyCode !== usdCurrencyCode && (
                  <div className="text-14 text-light-1">{currencyCode} {formatPrice(convertedPrice)}</div>
                )}
              </div>
            </div>
          </div>
          ...
          {/* <div className="row y-gap-5 justify-between pt-5">
            ...
            <div className="col-auto">
              <div className="text-15">
                ...
                {showSgdPortion && (
                  <div className="text-14 text-light-1">SGD {formatPrice(sgdTaxAmount)} ({sgdTax}%)</div>
                )}
              </div>
            </div>
          </div> */}
            ...
            <div className="row y-gap-5 justify-between">
              ...
              <div className="col-auto">
                <div className="text-18 lh-13 fw-500">
                  ...
                  {showSgdPortion && (
                    <div className="text-14 text-light-1">SGD {formatPrice(sgdGrandTotal)}</div>
                  )}
                </div>
              </div>
            </div>
