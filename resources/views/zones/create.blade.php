...
<script>
    $(document).ready(function() {
        ...
        const oldCountry = @json(old('country', $selectedCountry ?? $preselectedCountry ?? ''));
        ...
        // Reload cities if validation error / ensure list matches country
        if (oldCountry && !oldCityId) {
            loadCitiesByCountry(oldCountry, oldCityId);
        } else if (oldCountry && $('#city option').length <= 1) {
            loadCitiesByCountry(oldCountry, oldCityId);
        }
    });
</script>
