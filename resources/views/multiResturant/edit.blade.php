    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            <h5 class="card-header d-flex justify-content-between align-items-center">
                Edit Multi Restaurant Package
                <a href="{{ route('multiResturant.index') }}" class="btn btn-sm btn-outline-light">
                    <i class="fas fa-arrow-left me-1"></i> Back
                </a>
            </h5>
            <x-alert />
            <form id="multiRestaurantForm" method="POST" action="{{ route('multiResturant.update', Crypt::encrypt($multiRestaurant->id)) }}" class="card-body">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="package_name" name="package_name"
                               placeholder="Enter Package Name" value="{{ old('package_name', $multiRestaurant->package_name) }}" required>
                        <small class="text-muted">Package ID: {{ $multiRestaurant->package_id }}</small>
                        @error('package_name')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="status" name="status">
                            <option value="1" {{ (old('status', $multiRestaurant->status) == 1) ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ (old('status', $multiRestaurant->status) == 0) ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('status')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <select name="restaurants[]" id="restaurantsSelect" class="form-select" multiple required>
                            ...
                            @php
                                $selectedIds = array_map('intval', $multiRestaurant->getRestaurantsAsArray());
                            @endphp
                ...
                <div class="row">
                    <div class="col-12 text-center">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-2"></i> Update Multi Restaurant Package
                        </button>
                    </div>
                </div>
            </form>
...
<script>
    ...
    function updateRestaurantPreview() {
        ...
        selectedOptions.each(function() {
            ...
            var previewItem = $('<div class="restaurant-preview-item" data-id="' + id + '"><img src="' + src + '" alt="' + name + '"><div class="restaurant-info"><h6 class="mb-1">' + name + '</h6><p class="text-muted mb-0">ID: ' + id + '</p></div></div>');
