...
@section('title', 'Attraction Approval')
            ...
            <div class="card-datatable table-responsive pt-0">
                <div class="d-flex justify-content-between align-items-center" style="margin: 15px;">
                    <div class="d-flex align-items-center">
                        <h5 class="card-title mb-0">Attraction Approval</h5>
                    </div>
                ...
                <table class="datatables-basic table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th>Child Price</th> --}}
                            <th>location</th>
                            {{-- <th>Price with Transport (shared)</th>
                            <th>Price with Transport (private)</th> --}}
                            <th>Status</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($pendingattractions as $key => $pendingattraction)
                        <tr>
                            <td>{{ ++$key }}</td>
                                <td class="category-name">{{ $pendingattraction->name }}</td>
                                @php
                                    $roleId = auth()->user()->role_id;
                                @endphp
                                ...
                                @if($roleId == 10)
                                    @php
                                        $dmcUser = App\Models\User::where('userId', $pendingattraction->dmc_id)->first();
                                    @endphp
                                ...
                                @endif
                                {{-- <td>
                                    @if($pendingattraction->adult_price)
                                        {{ $pendingattraction->adult_price }}
                                    @else
                                        <span class="badge bg-danger">No details</span>
                                    @endif
                                </td>
                                <td>
                                @if($pendingattraction->child_price)
                                    {{$pendingattraction->child_price}}
                                @else
                                    <span class="badge bg-danger">No details</span>
                                @endif
                                </td> --}}
                                <td>
                                    {{$pendingattraction->location}}
                                </td>
                                {{-- <td>
                                    @if($pendingattraction->price_shared)
                                        {{ $pendingattraction->price_shared }}
                                    @else
                                        <span class="badge bg-danger">No details</span>
                                    @endif
                                </td>
                                <td>
                                    @if($pendingattraction->price_private)
                                        {{ $pendingattraction->price_private }}
                                    @else
                                        <span class="badge bg-danger">No details</span>
                                    @endif
                                </td> --}}
                            <td>
                                @if($pendingattraction->is_active == 1)
