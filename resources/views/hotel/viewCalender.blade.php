        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="HotelCloseDate" method="POST" action="{{ route('hotel_close_dates') }}">
                        @csrf
                        <input type="hidden" name="hotel_unique_id" value="{{$hotel_unique_id}}">
                        
                        <!-- Closed Days -->
                        <div class="mb-3">
                            <label for="hotel_closed_days"><strong>Closed Days</strong></label>
                            <select id="hotel_closed_days" class="form-control select2" name="hotel_closed_days[]" multiple required>
                                @php
                                    $closedDays = is_string($hotel->close_days) ? json_decode($hotel->close_days, true) : ($hotel->close_days ?? []);
                                @endphp
                        ...
                        <!-- Closed Dates -->
                        <div class="mb-3">
                            <label for="hotel_holiday_dates"><strong>Holiday Dates</strong></label>
                            <input type="text" class="form-control flatpickr-input" id="hotel_holiday_dates"
                                   name="hotel_holiday_dates" placeholder="Select holiday dates" required>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const closedDays = @json(is_string($hotel->close_days) ? json_decode($hotel->close_days, true) : ($hotel->close_days ?? []));
        const closedDates = @json(explode(',', $hotel->close_dates ?? ''));

        // Initialize Flatpickr for selecting close dates
        flatpickr("#hotel_holiday_dates", {
            mode: "multiple",
            dateFormat: "Y-m-d",
