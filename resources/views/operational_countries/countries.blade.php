            ...
            <div class="card-datatable table-responsive pt-0">
                <div class="d-flex justify-content-between align-items-center" style="margin: 15px;">
                    <div class="d-flex align-items-center">
                        <h5 class="card-title mb-0">City Charges</h5>
                    </div>

                    <div class="d-flex justify-content-between gap-3">
                        <!-- Add New Country Button -->
                        @if(hasPermission('create country'))
                            <a href="{{ route('country.create') }}" class="btn btn-primary btn-sm d-flex align-items-center gap-2">
                                <i class="fas fa-plus"></i> Add New City Charge
                            </a>
                        @endif
                    ...
                    <tbody>
                        @foreach($countries as $country)
                            <tr>
                                ...
                                <td class="city-name">{{ $country->city }}</td>
                                {{-- <td>{{ $country->country_currency }}</td>
                                <td>{{ $country->base_distance }} {{ $country->distance_unit }}</td> --}}
                                
                                <td>{{ $country->holiday_charges }}</td>
                                <td>{{ $country->peak_period_charge }}</td>
                                @if(hasPermission('edit country') || hasPermission('delete country'))
                                <td style="display: inline-block; white-space: nowrap;">
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="setDeleteForm('{{ route('country.destroy', $country->operational_country_id) }}')">
                                        <svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 -960 960 960" width="16px" fill="#ffffff">
