        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="DriverCloseDate" method="POST" action="{{ route('driver_close_dates') }}">
                        @csrf
                        <input type="hidden" name="driver_id" value="{{$driver_id}}">
                        
                        <!-- Closed Days -->
                        <div class="mb-3">
                            <label for="driver_closed_days"><strong>Closed Days</strong></label>
                            <select id="driver_closed_days" class="form-control select2" name="driver_closed_days[]" multiple required>
                                @php
                                    $closedDays = is_string($driver->close_days) ? json_decode($driver->close_days, true) : ($driver->close_days ?? []);
                                @endphp
                        ...
                        <!-- Closed Dates -->
                        <div class="mb-3">
                            <label for="driver_holiday_dates"><strong>Holiday Dates</strong></label>
                            <input type="text" class="form-control flatpickr-input" id="driver_holiday_dates"
                                   name="driver_holiday_dates" placeholder="Select holiday dates" required>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const closedDays = @json(is_string($driver->close_days) ? json_decode($driver->close_days, true) : ($driver->close_days ?? []));
        const closedDates = @json(explode(',', $driver->close_dates ?? ''));

        // Initialize Flatpickr for selecting close dates
        flatpickr("#driver_holiday_dates", {
            mode: "multiple",
            dateFormat: "Y-m-d",
