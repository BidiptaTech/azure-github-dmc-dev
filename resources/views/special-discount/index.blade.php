      ...
      <div class="card-datatable table-responsive pt-0">
            <div class="d-flex justify-content-between align-items-center" style="margin: 15px;">
                <div class="d-flex align-items-center">
                    <h5 class="card-title mb-0">Special Discount List</h5>
                </div>

                <div class="d-flex justify-content-between gap-3">
                    <!-- Add New User Button -->
                    @if(hasPermission('create users'))
                      <a href="{{ route('discount.create') }}" class="btn btn-primary btn-sm d-flex align-items-center gap-2">
                          <i class="fas fa-plus"></i> Add New Special Discount
                      </a>
                    @endif
        ...
        <table class="datatables-basic table table-bordered">
          ...
          <tbody>
            @foreach ($discounts as $key => $discount)
              <tr>
                <td>{{ ++$key }}</td>
                <td>{{ $discount->special_discount_id }}</td>
                <td>{{ $discount->discount_amount }}</td>
                <td>{{ $discount->discount_type }}</td>
                <td>{{ $discount->expiry }}</td>
                <td>{{ $discount->agent ? $discount->agent->name : 'N/A' }}</td>
                <td>
                  <div class="d-flex gap-2">
                    <!-- Edit Button -->
                      <a href="{{ route('discount.edit', $discount->special_discount_id) }}" 
                    ...
                    <button type="button" 
                            ...
                            onclick="setDeleteForm('{{ route('discount.destroy', $discount->special_discount_id) }}')">
                      <svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 -960 960 960" width="16px" fill="#ffffff">
