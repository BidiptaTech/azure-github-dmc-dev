...
@section('content')
@php
    $handoverPreviewBase = route('bookinglist.handoverChecklist.preview', ['tour_id' => $encryptedTourId]);
    $handoverQuery = [
        'logo_type' => $logoType ?? 'dmc',
    ];
    $handoverPdfBase = route('bookinglist.handoverChecklist.pdf', ['tour_id' => $encryptedTourId]);
@endphp
<style>
...
<div class="container-fluid">
    <div class="row mb-2 align-items-start">
        <div class="col-md-6">
            <h4 class="mb-1">Handover Checklist Preview</h4>
        ...
        <div class="col-md-6 text-md-end mt-2 mt-md-0">
            <div class="invoice-preview-actions d-flex flex-wrap align-items-center justify-content-md-end gap-2 gap-md-3">
                <a
                    href="{{ $handoverPdfBase }}?logo_type={{ urlencode($logoType ?? 'dmc') }}"
    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="invoice-preview-toolbar" role="toolbar" aria-label="Handover checklist branding">
                <div class="toolbar-segment">
                    ...
                    <div class="btn-group btn-group-sm" role="group" aria-label="Company branding">
                        <a href="{{ $handoverPreviewBase }}?{{ http_build_query(array_merge($handoverQuery, ['logo_type' => 'dmc'])) }}"
                           class="btn {{ ($logoType ?? 'dmc') === 'dmc' ? 'btn-success' : 'btn-outline-secondary' }}">DMC</a>
                        <a href="{{ $handoverPreviewBase }}?{{ http_build_query(array_merge($handoverQuery, ['logo_type' => 'agency'])) }}"
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body" style="padding: 0;">
                    <iframe
                        id="handoverChecklistIframe"
                        src="{{ $handoverPdfBase }}?preview=1&logo_type={{ urlencode($logoType ?? 'dmc') }}"
                        style="width: 100%; height: 900px; border: none;"
                        title="Handover checklist PDF preview"
                    ></iframe>
                </div>
            </div>
        </div>
