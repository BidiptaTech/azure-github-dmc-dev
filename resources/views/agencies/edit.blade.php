...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-md-8">
                    ...
                    <p class="mb-0 opacity-90">Update agency details and manage branch locations</p>
                </div>
        ...
        <form action="{{ route('agencies.update', $agency->agency_id) }}" method="POST" id="agencyForm" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-md-6 mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="wp_number" 
                                   name="wp_number" 
                                   value="{{ old('wp_number', $agency->wp_number) }}" 
                            ...
                            <select class="form-select select2 @error('country') is-invalid @enderror" 
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->name }}" 
                                            {{ old('country', $agency->country) == $country->name ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-6 col-md-6 mb-3">
                            ...
                            <select class="form-select select2 @error('city') is-invalid @enderror" 
                                    id="city" 
                                    name="city" 
                                    data-selected-city="{{ old('city', $agency->city) }}"
                                    required>
                                <option value="">Select city...</option>
                                @if(old('city', $agency->city))
                                    <option value="{{ old('city', $agency->city) }}" selected>{{ old('city', $agency->city) }}</option>
                                @endif
                            </select>
                            @error('city')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-lg-6 col-md-6 mb-3">
                            ...
                            <select class="form-select select2 @error('id_card_type') is-invalid @enderror" 
                                    id="id_card_type" 
                                    name="id_card_type" 
                                    data-selected-card-type="{{ old('id_card_type', $agency->id_card_type) }}">
                                <option value="">Select country first to load card types...</option>
                                @if(old('id_card_type', $agency->id_card_type))
                                    <option value="{{ old('id_card_type', $agency->id_card_type) }}" selected>{{ old('id_card_type', $agency->id_card_type) }}</option>
                                @endif
                            </select>
                            @error('id_card_type')
                                <div class="invalid-feedback">{{ $message }}</div>
            ...
            <!-- Existing Branch Sections -->
            <div id="branchesContainer">
                @if($agency->branches && count($agency->branches) > 0)
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-6 mb-3">
                                        ...
                                        <select class="form-select select2 branch-country" 
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{ $country->name }}" 
                                                        {{ old('branches.'.$index.'.country', $branch['country'] ?? '') == $country->name ? 'selected' : '' }}>
                                                    {{ $country->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-lg-6 col-md-6 mb-3">
                                        ...
                                        <select class="form-select select2 branch-city" 
                                                ...
                                                data-selected-city="{{ old('branches.'.$index.'.city', $branch['city'] ?? '') }}"
                                                required>
                                            <option value="">Select city...</option>
                                            @if(old('branches.'.$index.'.city', $branch['city'] ?? ''))
                                                <option value="{{ old('branches.'.$index.'.city', $branch['city'] ?? '') }}" selected>{{ old('branches.'.$index.'.city', $branch['city'] ?? '') }}</option>
                                            @endif
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-lg-6 col-md-6 mb-3">
                                        ...
                                        <input type="text" 
                                               ...
                                               value="{{ old('branches.'.$index.'.postal_code', $branch['postal_code'] ?? '') }}"
                                               placeholder="Enter postal code">
                                    </div>
                                    ...
                                    <div class="col-lg-6 col-md-6 mb-3">
                                        ...
                                        <select class="form-select select2 branch-card-type" 
                                                ...
                                                data-selected-card-type="{{ old('branches.'.$index.'.id_card_type', $branch['id_card_type'] ?? '') }}"
                                                required>
                                            <option value="">Select country first to load card types...</option>
                                            @if(old('branches.'.$index.'.id_card_type', $branch['id_card_type'] ?? ''))
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea class="form-control" 
                                                  ...
                                                  required>{{ old('branches.'.$index.'.address', $branch['address'] ?? '') }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="add-branch-section mb-4">
                ...
                <div class="mt-3">
                    <span class="branch-counter" id="branchCounter">{{ $agency->branches ? count($agency->branches) : 0 }} Branches Added</span>
                </div>
            </div>
            ...
            <div class="modern-card">
                <div class="card-body p-4">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h6 class="mb-1">Ready to update agency?</h6>
                            <small class="text-muted">Please review all information before saving changes</small>
                        </div>
                        <div class="col-md-4 text-md-end">
                            <button type="submit" class="btn btn-primary btn-modern me-2">
                                <i class="ri-save-line me-1"></i> Update Agency
                            </button>
...
$(document).ready(function() {
    let branchIndex = {{ $agency->branches ? count($agency->branches) : 0 }};
    ...
    // Initialize Select2 for existing branches
    $('.branch-country, .branch-city, .branch-card-type').each(function() {
        if ($(this).hasClass('branch-country')) {
            initializeSelect2(this, 'Search for country...');
        } else if ($(this).hasClass('branch-city')) {
            initializeSelect2(this, 'Search for city...');
        } else if ($(this).hasClass('branch-card-type')) {
            initializeSelect2(this, 'Search for card type...');
        }
    });
    ...
    // Initialize cities and card types for head office if country is already selected
    if ($('#country').val()) {
        // Add a small delay to ensure Select2 is fully initialized
        setTimeout(function() {
            loadCitiesForElement('#city', $('#country').val());
            loadCardTypesForElement('#id_card_type', $('#country').val());
        }, 100);
    }

    // Initialize existing branch city and card type selects
    $('.branch-country').each(function() {
        const countrySelect = $(this);
        const citySelect = countrySelect.closest('.branch-section').find('.branch-city');
        const cardTypeSelect = countrySelect.closest('.branch-section').find('.branch-card-type');
        
        if (countrySelect.val()) {
            // Add a small delay to ensure Select2 is fully initialized
            setTimeout(function() {
                loadCitiesForElement(citySelect, countrySelect.val());
                loadCardTypesForElement(cardTypeSelect, countrySelect.val());
            }, 100);
        }
        ...
        countrySelect.on('change', function() {
            const selectedCountry = $(this).val();
            loadCitiesForElement(citySelect, selectedCountry);
            loadCardTypesForElement(cardTypeSelect, selectedCountry);
        });
    });
    ...
    function loadCitiesForElement(citySelector, country) {
        if (country) {
            ...
            let currentValue = $(citySelector).find('option[selected]').val() || $(citySelector).val();
            
            // If no current value, get from data attribute (for initial load)
            if (!currentValue) {
                currentValue = $(citySelector).data('selected-city');
            }
                ...
                success: function(response) {
                    ...
                    if (response.success && response.cities && response.cities.length > 0) {
                        ...
                        if (currentValue && currentValue.trim() !== '') {
                            $(citySelector).val(currentValue);
                            $(citySelector).trigger('change.select2'); // Trigger Select2 change
                            
                            // Double check that the value is actually set
                            setTimeout(function() {
                                if ($(citySelector).val() !== currentValue) {
                                    $(citySelector).val(currentValue).trigger('change.select2');
                                }
                            }, 50);
                        }
    ...
    function loadCardTypesForElement(cardTypeSelector, country) {
        ...
        if (country) {
            ...
            // Store current selected value before clearing
            let currentValue = $(cardTypeSelector).find('option[selected]').val() || $(cardTypeSelector).val();
            
            // If no current value, get from data attribute (for initial load)
            if (!currentValue) {
                currentValue = $(cardTypeSelector).data('selected-card-type');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success && response.card_types && response.card_types.length > 0) {
                        ...
                        // Set the selected value if it exists
                        if (currentValue && currentValue.trim() !== '') {
                            $(cardTypeSelector).val(currentValue);
                            $(cardTypeSelector).trigger('change.select2'); // Trigger Select2 change
                            
                            // Double check that the value is actually set
                            setTimeout(function() {
                                if ($(cardTypeSelector).val() !== currentValue) {
                                    $(cardTypeSelector).val(currentValue).trigger('change.select2');
                                }
                            }, 50);
                        }
    ...
    $('#agencyForm').on('submit', function(e) {
        ...
        if (!isValid) {
            ...
            // Show loading state
            $(this).find('button[type="submit"]').html('<span class="loading-spinner"></span>Updating Agency...').prop('disabled', true);
        }
    });
