            ...
            <div class="card-datatable table-responsive pt-0">
                <div class="d-flex justify-content-between align-items-center" style="margin: 15px;">
                    <div class="d-flex align-items-center">
                        <h5 class="card-title mb-0">Roles Listing</h5>
                    </div>

                    <div class="d-flex justify-content-between gap-3">
                        <!-- Add New Role Button -->
                        @if(hasPermission('create roles'))
                            <a href="{{ route('roles.create') }}" class="btn btn-primary btn-sm d-flex align-items-center gap-2">
                ...
                <table class="datatables-basic table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th>User Type</th>
                            <th>Status</th>
                            {{-- @if(hasPermission('edit roles') || hasPermission('delete roles'))
                                <th>Action</th>
                            @endif --}}
                        </tr>
                    </thead>
                        ...
                        @foreach ($roles as $key => $role)
                            <tr>
                                ...
                                <td>
                                    @if($role->user_type == 1)
                                        Admin
                                    @elseif($role->user_type == 2)
                                        DMC
                                    @elseif($role->user_type == 3)
                                        Master Dmc
                                    @elseif($role->user_type == 4)
                                        Channel Partner
                                    @elseif($role->user_type == 5)
                                        Direct Agent
                                    @else
                                        Unknown
                                    @endif
