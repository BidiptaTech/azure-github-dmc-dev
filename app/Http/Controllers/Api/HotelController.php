    ...
    */
    public function details(Request $request)
    {
        ...
        $hotel_list = [
            ...
            'image' => $hotel->main_image ?? '',
            'tour_male' => $tours ? ($tours->male_count ?? 0) : ($request->query('tour_male') ?? 0),
            'tour_female' => $tours ? ($tours->female_count ?? 0) : ($request->query('tour_female') ?? 0),
            'tour_adult' => $tours ? (($tours->adult + $tours->child) ?? 0) : ($request->query('tour_adult') ?? 0),
            'all_pax'=> $tours ? (($tours->adult + $tours->child + $tours->infant) ?? 0) : ($request->query('all_pax') ?? 0),
            'tour_child'=> $tours ? (($tours->child - $filtered_child_count_above) ?? 0) : ($request->query('tour_child') ?? 0),
            'tour_child_adult' => $tours ? $filtered_child_count_above : ($request->query('tour_child_adult') ?? 0),
            'check_in_time' => $check_in_time,
            'check_out_time' => $check_out_time,
            'tour_infant'=> $tours ? ($tours->infant ?? 0) : ($request->query('tour_infant') ?? 0),
