    ...
    public function showPackage(Request $request){
        ...
        foreach ($bookings as $booking) {
            ...
            $booking_lists[] = [
                'booking_id' => $booking->booking_id,
                'package_id' => $booking->package_id,
                'package_title' => $package_info->title ?? 'N/A',
                'destination' => $package_info->destination ?? 'N/A',
                'total_price' => $booking_details->total_price ?? 0,
                'adult_count' => $booking_details->adult_count ?? 0,
                'child_count' => $booking_details->child_count ?? 0,
                'check_in' => $travel_dates->check_in ?? null,
                'check_out' => $travel_dates->check_out ?? null,
                'status' => $booking->status,
                'created_at' => $booking->created_at,
                'updated_at' => $booking->updated_at
            ];
        ...
        return response()->json([
            ...
            'total_bookings' => count($booking_lists)
        ]);
    }
}
